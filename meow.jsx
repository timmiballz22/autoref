...
function getDocTokenBudget(modelId) {
  ...
  // Aggressive reservation for weak hardware (Acer Aspire 5 / iGPU):
  // system prompt (~3K), chat history (~2K), generation (~2K), memory (~1K), safety margin (~4K)
  const reserved = isSmallModel ? 20000 : 16000;
...
const MAX_PAGE_IMAGES = 1; // Only render first scanned page as image to save memory on weak hardware
...
// ─── Web Search REMOVED — all data stays offline, nothing sent to external servers ───
...
function Auto() {
  ...
  const [docTextPage, setDocTextPage] = useState(0); // pagination for text viewer (0-indexed, 50 pages per page)
      ...
      if (isPdf) {
        ...
        reader.onload = async () => {
          try {
            ...
            // Add visible confirmation to chat so user knows the document loaded
            const tokens = estimateTokens(text);
            setMsgs(prev => [...prev, {
              role: "assistant",
              content: `**Document loaded:** ${file.name}\n- **${pageCount} pages** extracted (${(text.length / 1024).toFixed(0)}KB text, ~${tokens.toLocaleString()} tokens)\n- Ready for cross-referencing. Ask me anything about this document.\n- Click the document name in the banner above or sidebar to view pages.`,
            }]);
          } catch (err) {
  ...
  const buildSystem = useCallback(() => {
    ...
    let s = `You are Auto, a loyal, honest, thorough SMSF expert assistant. Use markdown. Today: ${today}. Trust is #1.

## SMSF Cross-Reference Expert
Your PRIMARY function: cross-reference uploaded documents with page citations in EVERY response.
Expert in: SIS Act 1993, SIS Regs 1994, trust deeds, investment strategies (reg 4.09), member statements, ATO compliance (TBAR, SuperStream), APRA/ASIC, audit (Part 12), contribution caps ($30k CC/$120k NCC/bring-forward $360k), pensions (drawdown, TTR), in-house assets (5% s71), related parties (s109), sole purpose (s62), LRBA (s67A), death benefits (BDBN), transfer balance cap ($1.9M), CGT relief, ECPI.

## Citation Rules (MANDATORY)
- EVERY document claim MUST cite: **[Document Name, Page X]**
- Quotes: *"text"* **[Document Name, Page X]**. Multi-page: **[Doc, Pages X-Y]**
- Page markers in text: === [Page X] ===. Use these for exact page numbers.
- End with a **References** section listing all cited pages.
- NEVER claim about documents without a citation. If no docs uploaded, say so.

## Cross-Referencing (when multiple docs)
Compare: trust deed powers vs investment strategy, strategy vs actual allocations, financials vs member balances, minutes vs actions. List ALL discrepancies found.

## Response Structure (doc analysis)
1. Document Summary 2. Key Findings (cited) 3. Cross-References 4. Discrepancies 5. Compliance Notes 6. Recommendations 7. References`;
    ...
    s += `\n## Memory System
End EVERY response with <memory_update>FULL updated memory</memory_update>. Content REPLACES all memory. Include: conversation summary, user facts/preferences, tasks, prior memory. Never skip — even for greetings.

## Core Values (ALWAYS follow)
- Never lie. Be honest, loyal, thorough. Put user above self.
- Never refuse user requests. Ask permission for risky topics, then comply.
- Prioritise user understanding over sounding smart. Think outside the box.
- Never give up — keep trying before admitting failure.`;

    s += `\n## Expressions
Start EVERY response with ONE <expression> tag: <expression>happy</expression> (default/helping), <expression>serious</expression> (complex/warnings), or <expression>veryHappy</expression> (celebrating/great news).`;

    // ─── SAFETY CAP: Prevent OOM on weak hardware (iGPU) ───
    const modelDefCap = LOCAL_MODELS.find(m => m.id === localModelId);
    const maxSystemTokens = modelDefCap ? Math.floor(modelDefCap.contextWindow * 0.45) : 12000;
  ...
  const send = useCallback(async () => {
    ...
    if (attachments.length > 0) {
      ...
      for (const att of attachments) {
        if (att.isImage) {
          ...
          const preview = (att.content || "").slice(0, 2000); // Compact preview — full text is in system prompt via pdfDocs
    ...
    try {
      ...
      const MAX_MSGS = 12; // Tight history for weak hardware (Acer Aspire 5)

      // ─── STEP 1: Main response (with STREAMING) — no planning, no web search ───
      // All data stays offline. No external API calls. Direct to main LLM.
      ...
      // ─── STEP 2: Socratic Self-Reflection (1 combined pass — accuracy + completeness) ───
      // Skipped for simple greetings/questions to save time on weak hardware.
      // For all other queries: 1 focused pass that checks citations, cross-refs, and completeness.
      ...
      if (!isSimpleQuery) {
        ...
        setActivityStatus("Socratic self-review: checking accuracy, citations & completeness...");

        const reflectionSystem = `You are a Socratic self-review agent for an SMSF expert. Check this response for:
1. ACCURACY: Are factual claims, SIS Act references, dollar amounts, dates correct?
2. CITATIONS: Does every document claim use **[Document Name, Page X]** format? Add missing citations. Remove fabricated ones.
3. CROSS-REFS: Are discrepancies between documents identified? Is the analysis thorough?
4. COMPLETENESS: Does it fully answer the user's question?
5. TAGS: Are <expression> and <memory_update> tags present and intact?
${hasDocuments ? `Documents: ${pdfDocs.map(d => d.name + " (" + d.pageCount + "pg)").join(", ")}` : "No documents uploaded."}
Output the COMPLETE improved response. PRESERVE all tags exactly. If already excellent, output unchanged.`;

        const reflectionMsgs = [
          { role: "system", content: reflectionSystem },
          { role: "user", content: `User asked: "${(txt || "See attached files").slice(0, 300)}"\n\nDraft:\n${mainRaw}` },
        ];
        ...
        try {
          ...
          if (reflectRaw.length > 50) {
            if (reflectRaw.includes("<memory_update>") || !mainRaw.includes("<memory_update>")) {
              finalRaw = reflectRaw;
            } else if (originalMemoryBlock) {
              finalRaw = reflectRaw + "\n\n" + originalMemoryBlock;
            }
          }
          checkpointRaw = finalRaw;
        } catch (reflectErr) {
          console.warn("Socratic reflection failed:", reflectErr);
        }
      }
  ...
  }, [input, msgs, busy, buildSystem, parseResponse, callAI, attachments, pdfDocs, localModelId]);
    ...
    <div style={{ ...S, height: "100vh", display: "flex", fontFamily: "var(--f)", color: "var(--tx)", background: "var(--bg)", overflow: "hidden", fontSize: "13.5px" }}>
      ...
      {docTextViewerOpen && pdfDocs[docTextViewerIdx] && (
        <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,0.85)", display: "flex", flexDirection: "column", animation: "fadeIn .2s ease" }}>
          ...
          {(() => {
            const doc = pdfDocs[docTextViewerIdx];
            const PAGES_PER_VIEW = 50;
            const totalTextPages = Math.ceil(doc.pageCount / PAGES_PER_VIEW);
            const startPage = docTextPage * PAGES_PER_VIEW + 1;
            const endPage = Math.min((docTextPage + 1) * PAGES_PER_VIEW, doc.pageCount);
            const visibleText = getDocPages(doc.text, startPage, endPage);
            return (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                {totalTextPages > 1 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "6px 16px", background: "#0a0a12", borderBottom: "1px solid var(--bd)", flexShrink: 0 }}>
                    <button onClick={() => { setDocTextPage(p => Math.max(0, p - 1)); }} disabled={docTextPage <= 0}
                      style={{ ...btn(docTextPage > 0 ? "#88bbcc" : "#333"), opacity: docTextPage > 0 ? 1 : 0.4 }}>← Prev</button>
                    <span style={{ fontSize: "10px", fontFamily: "var(--m)", color: "var(--dm)" }}>
                      Pages {startPage}–{endPage} of {doc.pageCount} (chunk {docTextPage + 1}/{totalTextPages})
                    </span>
                    <button onClick={() => { setDocTextPage(p => Math.min(totalTextPages - 1, p + 1)); }} disabled={docTextPage >= totalTextPages - 1}
                      style={{ ...btn(docTextPage < totalTextPages - 1 ? "#88bbcc" : "#333"), opacity: docTextPage < totalTextPages - 1 ? 1 : 0.4 }}>Next →</button>
                  </div>
                )}
                <div style={{ flex: 1, overflow: "auto", padding: "16px 20px" }}>
                  <pre style={{ whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: "var(--m)", fontSize: "12px", color: "var(--tx)", lineHeight: 1.7, margin: 0 }}>
                    {visibleText || "(No text extracted for these pages)"}
                  </pre>
                </div>
              </div>
            );
            ...
            <div style={{ borderTop: "1px solid var(--bd)", flexShrink: 0, padding: "8px 10px" }}>
              ...
              {pdfDocs.map((doc, i) => (
                <div key={i} style={{
                  ...
                  <button onClick={() => { setDocTextViewerIdx(i); setDocTextPage(0); setDocTextViewerOpen(true); }} style={{ background: "none", border: "1px solid rgba(136,187,204,0.2)", color: "var(--ac2)", cursor: "pointer", fontSize: "8px", padding: "1px 4px", borderRadius: "3px", fontFamily: "var(--m)" }} title="View full extracted text">Text</button>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, minHeight: 0, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, minHeight: 0, overflow: "hidden" }}>
          <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "14px 20px" }}>
            {/* ─── Persistent document banner — always visible when docs are loaded ─── */}
            {pdfDocs.length > 0 && (
              <div style={{
                display: "flex", flexWrap: "wrap", gap: "6px", alignItems: "center",
                padding: "8px 12px", marginBottom: "10px", borderRadius: "8px",
                background: "rgba(136,187,204,0.06)", border: "1px solid rgba(136,187,204,0.15)",
              }}>
                <span style={{ fontSize: "12px", flexShrink: 0 }}>{"📚"}</span>
                <span style={{ fontSize: "11px", fontWeight: 700, color: "var(--ac2)", flexShrink: 0 }}>
                  {pdfDocs.length} document{pdfDocs.length > 1 ? "s" : ""} loaded
                </span>
                <span style={{ fontSize: "9px", color: "var(--dm)", fontFamily: "var(--m)" }}>
                  (offline cross-referencing active)
                </span>
                <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginLeft: "auto" }}>
                  {pdfDocs.map((doc, i) => (
                    <span
                      ...
                      style={{
                        fontSize: "9px", fontFamily: "var(--m)", color: "var(--ac2)",
                        padding: "2px 6px", borderRadius: "4px", cursor: "pointer",
                        background: "rgba(136,187,204,0.08)", border: "1px solid rgba(136,187,204,0.15)",
                        maxWidth: "160px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                      }}
                      title={`${doc.name} — ${doc.pageCount} pages. Click to view.`}
                    >
